...
class XUploadAction extends CAction {
     ...
     * @since 0.5
     */
    public $formClass = 'application.extensions.zoomupload.models.XUploadForm';
    ...
    public function init( ) {
        ...
        $this->_subfolder=Yii::app()->getRequest()->getQuery('pid'); // Set subfolder
    ...
    protected function handleDeleting()
    {
        if (isset($_GET["_method"]) && $_GET["_method"] == "delete") {
			...
			$row = Yii::app()->db->createCommand()->from('xlsws_mod_zoomproduct')->where('product_id=:id AND name=:name', array(':id'=>$pid,':name'=>$_GET["file"]))->queryRow(); // Get row will delete to check level
			$delRow = Yii::app()->db->createCommand()->delete('xlsws_mod_zoomproduct', 'product_id='.$pid.' AND name="'.$_GET["file"].'"'); // Delete row when del image
			if($row['level']==1){
				$rowUpdate = Yii::app()->db->createCommand()->from('xlsws_mod_zoomproduct')->where('product_id=:id', array(':id'=>$pid))->queryRow(); // Get a row default
				if($rowUpdate){
					$update = Yii::app()->db->createCommand()->update('xlsws_mod_zoomproduct',array('level'=>1),'id='.$rowUpdate['id']); // Update row default
				}
			}
			$pathfile=Yii::getPathOfAlias('webroot').'/images/zoomproduct/'.$pid.'/'.$_GET["file"];	// Create path
            if (is_file($pathfile)){
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->validate()) {
				...
				$pid=Yii::app()->getRequest()->getQuery('pid');
				$row = Yii::app()->db->createCommand()->from('xlsws_mod_zoomproduct')->where('product_id=:id AND level=:level', array(':id'=>$pid,':level'=>1))->queryRow();
				if(!$row){
					$insert = Yii::app()->db->createCommand()->insert('xlsws_mod_zoomproduct',array('product_id'=>$pid,'name'=>$model->{$this->fileNameAttribute},'url'=>'/zoomproduct/'.$pid.'/'.$model->{$this->fileNameAttribute},'size'=>$model->size,'type'=>0,'level'=>1));
				}else{
					$insert = Yii::app()->db->createCommand()->insert('xlsws_mod_zoomproduct',array('product_id'=>$pid,'name'=>$model->{$this->fileNameAttribute},'url'=>'/zoomproduct/'.$pid.'/'.$model->{$this->fileNameAttribute},'size'=>$model->size,'type'=>0,'level'=>0));
                }
				} else {
                    echo json_encode(array(array("error" => $returnValue,)));
                    Yii::log("XUploadAction: " . $returnValue, CLogger::LEVEL_ERROR, "application.extensions.zoomupload.actions.XUploadAction");
                }
            } else {
                echo json_encode(array(array("error" => $model->getErrors($this->fileAttribute),)));
                Yii::log("XUploadAction: " . CVarDumper::dumpAsString($model->getErrors()), CLogger::LEVEL_ERROR, "application.extensions.zoomupload.actions.XUploadAction");
            }
        } else {
